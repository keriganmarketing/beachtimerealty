...
use Includes\Modules\CPT\CheckToRadio;
use Includes\Modules\CPT\TaxonomyMeta;
	...
	public function convertCheckToRadio( $tax ) {
		add_filter( 'wp_terms_checklist_args', function ( $args ) use ( $tax ) {
			if ( ! empty( $args['taxonomy'] ) && $args['taxonomy'] === $tax ) {
				if ( empty( $args['walker'] ) || is_a( $args['walker'],
						'Walker' ) ) { // Don't override 3rd party walkers.
